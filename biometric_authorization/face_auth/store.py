...
class EnrollmentStore:
    def __init__(self, db_path: Path | None = None, *, embedding_dim: int | None = None):
        self.db_path = Path(db_path) if db_path else ENROLL_DB_PATH
        self.embedding_dim = int(embedding_dim) if embedding_dim is not None else EMBEDDING_DIM
    ...
    def upsert(self, user_id: str, embedding: np.ndarray, sample_count: int = 1) -> None:
        if embedding.shape != (self.embedding_dim,):
            raise ValueError(f"embedding must be ({self.embedding_dim},), got {embedding.shape}")
        if sample_count < 1:
